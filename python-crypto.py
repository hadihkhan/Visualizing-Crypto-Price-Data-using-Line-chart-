...
class cryptoVisual:
    def __init__(self, sym, comp_sym, all_data=False, limit=1000, aggregate=1, exchange=''):
        self.symbol = sym
        ...
        self.url = 'https://min-api.cryptocompare.com/data/histoday?fsym={}&tsym={}&limit={}&aggregate={}' \
            .format(self.symbol.upper(), self.comp_sym.upper(), self.limit, self.aggregate)
        if self.exchange:
    ...
    def plotGraph(self):
        ...
        plt.title(self.symbol + ' To ' + self.comp_sym, fontsize=24)
        plt.ylabel('Price In ' + self.comp_sym, fontsize=18)
        plt.xlabel('Year', fontsize=18)
        plt.show()


BTC = cryptoVisual('BTC', 'USD', all_data=True)
BTC.plotGraph()
